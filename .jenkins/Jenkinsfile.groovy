...
def testImage(unrealVersion, expectedTag) {
    docker.image(candidateImage()).inside {
        exec "powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .jenkins/TestImage.ps1 -UnrealVersion ${unrealVersion} -ExpectedTag ${expectedTag}"
    }
}
