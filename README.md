...
This repository builds one Windows Docker image that installs and compiles the requested Unreal Engine minor release when an engine-backed `Unreal` command is first invoked. Source, caches, and Installed Builds live in named volumes and are reused by later containers.
...
The image exposes five public command forms:
...
`Build`, `RunUAT`, and `Cmd` pass all following arguments to the selected engine tool and return its native exit code. `RunUAT` initializes the engine's bundled .NET environment before invoking `Engine/Build/BatchFiles/RunUAT.bat`. `--version` prints the full version of the compiled engine selected by `UNREAL_VERSION`; `--help` lists every command and its usage.

## Runtime installation

The first engine-backed `Unreal` invocation performs the following work:
...
7. Dispatch to that installation's `Build.bat`, `RunUAT.bat`, or `UnrealEditor-Cmd.exe` with the original arguments and return its exit code.

Every engine-backed invocation resolves the branch again. If the exact commit is already published, the launcher does not touch a source worktree or invoke the compiler. If the branch has moved, it cleans and rebuilds only that minor's worktree while retaining other minor worktrees and the last complete installation, then replaces that installation atomically. A failed update therefore does not destroy the previous engine.
