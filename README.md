...
- `tag` is the default. It considers tags whose complete names are valid [Semantic Versioning 2.0.0](https://semver.org/spec/v2.0.0.html) versions with major and minor components equal to `UNREAL_VERSION`. A tag with prerelease identifiers is excluded unless its prerelease consists of the single identifier `release`. The remaining tags are ordered by SemVer precedence and the greatest one is selected. For example, `UNREAL_VERSION=5.8` selects `5.8.2-release` over both `5.8.1-release` and `5.8.0`, while tags such as `5.8.3-preview-1` are ignored. The identifier is the complete tag name, while the selected tag is still resolved to its commit for checkout and installation validation.
- `branch` enables the rolling behavior. It resolves the exact `refs/heads/<minor>` branch, such as `refs/heads/5.8`, to its current commit. The identifier is the full commit hash.
...
1. Resolve the configured branch or highest eligible semantic-version tag at `UNREAL_SOURCE` and retain both its public identifier and commit.
...
6. Atomically publish the completed engine under `C:/unreal/binaries/<minor>` and record its source URL, requested minor, resolution mode, identifier, commit, patch version, and Installed Build profile.
